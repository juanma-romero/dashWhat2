...
async def listar_pedidos_pendientes(limite: int = 10):
    ...
    try:
        ...
        if isinstance(e, HTTPException):
            raise e
        print(f"Error inesperado listando pedidos: {e}")
        raise HTTPException(status_code=500, detail=f"Un error inesperado ocurrió: {e}")


@app.post("/crear-pedido/", summary="Crear un Nuevo Pedido de Venta")
...
@app.get("/buscar-cliente/{telefono}", summary="Buscar Cliente por Teléfono")
